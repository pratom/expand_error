Function describe_error_briefly
{
    [cmdletbinding()] Param (
    ...
    try 
    {
        ...
        $format_err = New-Module {            
                        import-module "$PSScriptRoot\error_formatter.psm1" -force -noclobber   
        ...
        { 
            ...
            Default 
                {
                    return "The function, describe_error_briefly, will not attempt to describe anything as the the parameter set passed to it is unknown to it.$($nl)$($nl)"   
                }            
        } 
        ...
        write-debug "describe_error_briefly was asked to an error object thingee of some sort.  Here is what it said:$($nl)$desc"
        if ($desc -eq $null) { $desc = "Hmmm.....We describe this error object as NULL ???"}
        return $desc
    }
    catch 
    {  
        ...
        $err_msg = "The function, describe_error_briefly, itself threw an exception, while attempting to describe the exception information sent to it. ITS Exception information=[$internal_err]"
        write-warning "ERROR ERROR ERROR $($err_msg)"
        return  $err_msg   
    }    
}
